...
function checkChar(charCode: number, rangeList: RANGE[]) {
  let i;
  let j;
  let start;
  let end;
  for ((i = 0), (j = rangeList.length); i < j; i++) {
    ...
    if (end === null && charCode === start) {
      return true;
    } else if (end !== null && end !== undefined && charCode >= start && charCode <= end) {
      return true;
    }
  }
  return false;
...
function isCJK(charCode: number) {
  // eslint-disable-next-line max-len
  const CJKRange: RANGE[] = [[0x3040, 0x318f], [0x3300, 0x337f], [0x3400, 0x3d2d], [0x4e00, 0x9fff], [0xf900, 0xfaff], [0xac00, 0xd7af]];
  return checkChar(charCode, CJKRange);
}


function isSpace(charCode: number) {
  const SpaceRange: RANGE[] = [[0x0009, 0x000d], [0x0020], [0x00a0]];
  return checkChar(charCode, SpaceRange);
}


function isAlpha(charCode: number) {
  // eslint-disable-next-line max-len
  const AlphaRange: RANGE[] = [[0x0030, 0x0039], [0x0041, 0x005a], [0x0061, 0x007a], [0x00c0, 0x00d6], [0x00d8, 0x00f6], [0x00f8, 0x100]];
  return checkChar(charCode, AlphaRange);
}

function isNormalAlpha(char: string) {
...
function isNonCjkAlpha(char: string) {
  const charCode = char.charCodeAt(0);
  if ((charCode >= 48 && charCode <= 57) || charCode === 46) {
    return true;
  }
...
function processCJKSearchKeywords(keywords: string) {
  if (!keywords) {
    return '';
  }
...
function highlightTextEx(text: string, keywords: string, splitCjk = true, allowFromMiddle = false, options: {
  full?: boolean,
} = {}) {
  //
  if (!text) {
  ...
  // 过滤无效字符
  const filterWords = (keywords: string) => {
    let processKeywords = '';
    let inQuot = false;
    for (let i = 0; i < keywords.length; i++) {
      //
  ...
  //
  // 在cjk和英文字母之间添加空格
  const appSpaceBetweenAlphaAndCJK = (keywords: string) => {
    let processKeywords = '';
  ...
  for (;;) {
    ...
    //
    if (orgIndexArr.length >= 10) {
      break;
    }
  }
  //
  if (orgIndexArr.length === 0) {
    return null;
  }
  // 结果进行排序
  orgIndexArr.sort((a, b) => a.index - b.index);
  //
  //
  // 根据搜索结果位置，提取上下文
  const extractText = (index: number) => {
    //
    if (options.full) {
      return {
        index,
    ...
    //
    const isSentenceEnd = (text: string, index: number) => {
  ...
  let filteredIndexArr: {
    ...
    search?: string,
    skip?: boolean,
  }[] = orgIndexArr.map((item) => Object.assign(item, extractText(item.index)));
  //
  // 合并临近的关键字
  for (let i = filteredIndexArr.length - 2; i >= 0; i--) {
    //
    const cur = filteredIndexArr[i];
    const next = filteredIndexArr[i + 1];
  ...
  // 去掉被合并的结果
  filteredIndexArr = filteredIndexArr.filter((item) => !item.skip);
  //
  // 搜索结果进行高亮
  for (const item of filteredIndexArr) {
    //
    assert(item.search);
    // 进行最大匹配，不完美，可以优化
  ...
  //
  const result = filteredIndexArr.map((item) => item.context);
  //
  if (result.length === 0) {
    return null;
  }
...
function highlightText(text: string, keywords: string, options: { allowFromMiddle?: boolean, full?: boolean } = {}) {
  //
